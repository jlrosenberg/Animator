...
public class Motion implements Comparable{
    ...
    public Motion(KeyFrame start, KeyFrame end) {
        if(start.getTick()>end.getTick()){
            throw new IllegalArgumentException("Cannot create a motion object with a start KeyFrame that " +
                    "occurs after the end KeyFrame");
        }
        this.start = start;
        this.end = end;
    }
    ...
    public boolean inAction(int time){
        if(time>=start.getTick()&&time<=end.getTick()){
            return true;
        }
    ...
    public int compareTo(Object o) {
        if(o instanceof Motion){
            return this.start.compareTo(((Motion)o).getStart());
        }
