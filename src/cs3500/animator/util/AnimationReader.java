...
public class AnimationReader {
   ...
   * <li>Motion lines: the keyword "motion" followed by an identifier giving the name of the shape to move,
   * and 16 integers giving the initial and final conditions of the motion:
   * eight numbers giving the time, the x and y coordinates, the width and height,
   * and the red, green and blue color values at the start of the motion; followed by eight numbers
   * for the end of the motion.  See {@link AnimationBuilder#addMotion}</li>
   * </ul>
   * </p>
   *
  ...
  public static <Doc> Doc parseFile(Readable readable, AnimationBuilder<Doc> builder) {
    ...
    s.useDelimiter(Pattern.compile("(\\p{Space}+|#.*)+")); // Split at whitespace, and ignore # comment lines
    while (s.hasNext()) {
      String word = s.next();
      switch(word) {
        case "canvas":
  ...
  private static int getInt(Scanner s, String label, String fieldName) {
    if (s.hasNextInt()) {
      ...
      throw new IllegalStateException(
              String.format("%s: expected integer for %s, but no more input available", label, fieldName));
    }
  }
